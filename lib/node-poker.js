	...
	if(table.game){
		if(CheckForEndOfRound(table) == true){
			...
			if(table.game.roundName == "Turn"){
				...
				table.game.board[i] = table.game.deck.pop(); //Turn a card
				table.game.bets.splice(0,table.game.bets.length);
			};
		...
		if(table.players.length >= table.minPlayers){
			...
			for(x=0;x<2;x++){
				for(i=0;i<table.players.length;i++){
					table.players[i].cards[x] = table.game.deck.pop();
				};
			};
			//Identify Small and Big Blind player indexes
			var smallBlind = table.dealer + 1;
			if(smallBlind >= table.players.length){
				smallBlind = 0;
			};
			var bigBlind = table.dealer + 2;
			if(bigBlind >= table.players.length){
				bigBlind -= table.players.length
			};
			//Force Blind Bets
			table.players[smallBlind].chips -= table.smallBlind;
			table.players[bigBlind].chips -= table.bigBlind;
			table.game.bets.push(table.smallBlind);
			table.game.bets.push(table.bigBlind);
		};
	};
};
