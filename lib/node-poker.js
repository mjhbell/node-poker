...
Player.prototype.Check = function(){
    var checkAllow = true
    for(var v=0;v<this.table.game.bets.length;v++){
...
Player.prototype.Bet = function(bet){
    //TODO Manage allin (check if bet < chips)
    if(this.chips > bet) {
        for(var i=0;i<this.table.players.length;i++){
            if(this == this.table.players[i]){
...
function CheckForWinner(table){
    ...
    for(var l=0;l<table.game.roundBets.length;l++){
    ...
    var roundEnd = true;
    for(l=0;l<table.game.roundBets.length;l++){
	...
	if(table.game){
		if(CheckForEndOfRound(table) == true){
			...
			for(var i=0;i<table.game.bets.length;i++){
                ...
                table.game.roundBets[i] += parseInt(table.game.bets[i]);
			};
